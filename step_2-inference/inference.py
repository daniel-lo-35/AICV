...
def main(args):
    ...
    for f in files:
        ...
        # 切照片 
        # 參數1: 小照片重疊率
        if args.overlap_pixel > 1 or args.overlap_pixel == 0:
            overlap_pixel_x = int(args.overlap_pixel)
            overlap_pixel_y = int(args.overlap_pixel)
        elif 0 < args.overlap_pixel < 1:
            overlap_pixel_x = int(1280*args.scale * args.overlap_pixel)
            overlap_pixel_y = int(720*args.scale * args.overlap_pixel)
        else:
            print("Input of overlap_pixel (", args.overlap_pixel, ") not supported! Change back to default value (280)")
            overlap_pixel_x = 280
            overlap_pixel_y = 280
        x_window_size = int(1280*args.scale) - overlap_pixel_x
        y_window_size = int(720*args.scale) - overlap_pixel_y
        im_list = [image[y:y+int(720*args.scale), x:x+int(1280*args.scale)] for y in range(0, image.shape[0], y_window_size) for x in range(0, image.shape[1], x_window_size)]
        ...
        if len_masks > 1:
            ...
            if junctions.shape[0] > 0 and enpoints.shape[0] > 0:
                ...
                vis_link_group_combined(image, _group_horizontal, _group_vertical, args, f)
                if length_per_pixel is not None:
                    vis_group_spacing_combined(image, _group_horizontal, _group_vertical, result_horizontal, result_vertical, args, f)
                
                show_link_group_combined(image, _group_horizontal, _group_vertical, args, f)
                if length_per_pixel is not None:
...
def ActiveInspection(args, info, filename, length_per_pixel=None):
    ...
    for orien in orientations:
        if orien > horizontal_orientation - 3 * horizontal_std and orien < horizontal_orientation + 3 * horizontal_std:     # Check 68-95-99.7 rule. I use 2 sigma = 95% # Change to 3
            horizontal_group.append(orien)
        elif orien > vertical_orientation - 3 * vertical_std and orien < vertical_orientation + 3 * vertical_std:    # Check 68-95-99.7 rule. I use 2 sigma = 95% # Change to 3
...
def double_gaussian(args, data, filename):
    ...
    y, x, _ = plt.hist(np.asarray(data), max(len(data)//4, 1), alpha=.3, label='data')
...
def find_link_group(dimension, group, max_e2e_dist=20, min_e2e_dist=1, search_limit=None):
...
def vis_link_group_combined(background, group1, group2, args, filename, junc_color=(0, 0, 255), link_color=(0, 127, 0), unit="cm"):
    start_time = time.time()
    print("visualizing link group (combined)...")
    background = background.astype(np.uint8)
    ...
    for i, info in tqdm(group1.items()):
        line = LineString([pt for pts in info for pt in pts["points"]])
        x, y = line.xy
        plt.plot(x, y, linewidth=3)
        plt.text(x[0], y[0], str(i))

    for i, info in tqdm(group2.items()):
        line = LineString([pt for pts in info for pt in pts["points"]])
        x, y = line.xy
        plt.plot(x, y, linewidth=3)
        plt.text(x[0], y[0], str(i))
    ...
    plt.savefig(os.path.join(args.output_dir, filename + '-linegroup-combined.jpg'), bbox_inches='tight', pad_inches = 0)

    end_time = time.time()
    print('Time for visualizing line group:', end_time-start_time, 'seconds')

def show_link_group_combined(background, group1, group2, args, filename, junc_color=(0, 0, 255), link_color=(0, 127, 0), unit="cm"):
    start_time = time.time()
    print("Displaying link group (combined)...")
    background = background.astype(np.uint8)
    ...
    plt.savefig(os.path.join(args.output_dir, filename + '-linegroup-display.jpg'), bbox_inches='tight', pad_inches = 0)

    end_time = time.time()
    print('Time for displaying line group:', end_time-start_time, 'seconds')

def vis_group_spacing_combined(background, group_horizontal, group_vertical, report_horizontal, report_vertical, args, filename, junc_color=(0, 0, 255), link_color=(0, 127, 0), unit="cm"):
    start_time = time.time()
    print("Visualizing group of spacings (combined)...")
    background = background.astype(np.uint8)
    ...
    for i, info in tqdm(report_horizontal.items()):
        ...
        # x, y = line.xy
        # plt.plot(x, y, linewidth=3)
        x0, y0 = group_horizontal[i][0]["location"]
        x1, y1 = group_horizontal[info["connect_line_id"]][0]["location"]
        plt.annotate("", xy=(x0, y0), xytext=(x1, y1), arrowprops=dict(connectionstyle="arc3, rad=0.1", arrowstyle="<|-|>"))
        color = "r" if info["outliers"] else "w"
        plt.text((x0+x1)/2, (y0+y1)/2, "{:.2f} +- {:.2f} cm".format(info["mean"], info["std"]), fontsize="large", verticalalignment="center", horizontalalignment="center", bbox=dict(boxstyle="Round", ec="None", fc=color, alpha=0.8))
    ...
    for i, info in tqdm(report_vertical.items()):
        ...
        # x, y = line.xy
        # plt.plot(x, y, linewidth=3)
        x0, y0 = group_vertical[i][0]["location"]
        x1, y1 = group_vertical[info["connect_line_id"]][0]["location"]
        plt.annotate("", xy=(x0, y0), xytext=(x1, y1), arrowprops=dict(connectionstyle="arc3, rad=0.1", arrowstyle="<|-|>"))
        color = "r" if info["outliers"] else "w"
        plt.text((x0+x1)/2, (y0+y1)/2, "{:.2f}\n+-{:.2f}cm".format(info["mean"], info["std"]), fontsize="large", verticalalignment="center", horizontalalignment="center", bbox=dict(boxstyle="Round", ec="None", fc=color, alpha=0.8))
    ...
    # plt.imshow(np.flip(background, axis=0))    # I use np.flip because y's direction in array image is up to down, while that in pyplot plot line is down to up    # <- forget that = =
    # plt.title("{} : group of spacing".format(direction))
    plt.savefig(os.path.join(args.output_dir, filename + '-spacing_group-combined.jpg'), bbox_inches='tight', pad_inches = 0)

    end_time = time.time()
    print('Time for visualizing group of spacing:', end_time-start_time, 'seconds')

def show_group_spacing_combined(background, group_horizontal, group_vertical, report_horizontal, report_vertical, args, filename, junc_color=(0, 0, 255), link_color=(0, 127, 0), unit="cm"):
    start_time = time.time()
    print("Displaying group of spacings (combined)...")
    background = background.astype(np.uint8)
    ...
    # plt.imshow(np.flip(background, axis=0))    # I use np.flip because y's direction in array image is up to down, while that in pyplot plot line is down to up    # <- forget that = =
    # plt.title("{} : group of spacing".format(direction))
    plt.savefig(os.path.join(args.output_dir, filename + '-spacing_group-display.jpg'), bbox_inches='tight', pad_inches = 0)
...
def create_dxf(background, group_horizontal, group_vertical, report_horizontal, report_vertical, args, filename):
    ...
    doc.saveas(os.path.join(args.output_dir, filename + '.dxf'))

def create_dxf_r12writer(background, group_horizontal, group_vertical, report_horizontal, report_vertical, args, filename):
    with r12writer(os.path.join(args.output_dir, filename + '.dxf')) as dxf:
        horizontal_count = len(report_horizontal)
        vertical_count = len(report_vertical)
